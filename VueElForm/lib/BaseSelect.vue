...
<template>
  <el-select
    ...
    :remote-method="query=>options['remote-method'](query,options)"
  ...
  >
    ...
    <template v-if="options['options']">
      <div v-if="!options['options'].length">
        <el-option
          v-for="(val, key,index) in options['options']"
          :key="index"
          :value="options.optionToggle?val:key"
          :label="options.optionToggle?key:val"
        />
      </div>
      <div v-else>
        <el-option
          v-for="option in options['options']"
          :key="option[options.valueKey||'value']||option"
          :value="option[options.valueKey||'value']||option"
          :label="option[options.labelKey||'label']||option"
        />
      </div>
    </template>
